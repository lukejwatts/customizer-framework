...
use CustomizerFramework\Control\Image_Checkbox_Control;
 ...
 * @version 1.0.0
 */
final class Field_Image_Checkbox extends Settings
{
	/**
	...
	public function render( $wp_customize, $config ) {
		...
		$field_name =  \CustomizerFramework\error_field_name( 'image-checkbox', $config['id'] );
		...
		if ( is_array( $args ) && parent::sanitize_argument( $config, $field_name ) != false && $is_valid_choices == true && $is_valid_direction == true && $is_valid_size == true && $is_valid_default == true ) {
			...
			$wp_customize->add_control( new Image_Checkbox_Control( $wp_customize, $args['id'] . '_field', array(
