 ...
 * @version 1.0.0
 */
final class Field_Email extends Settings
{
	/**
	...
	private function is_valid_default( $default, $field ) {
		if ( ! empty( $default ) ) {
			if ( filter_var( $default, FILTER_VALIDATE_EMAIL ) == false ) {
				\CustomizerFramework\alert_warning( 'Error 315: invalid Email in '. \CustomizerFramework\code( 'error', 'default' ) .' at field '. \CustomizerFramework\code( 'success', $field ) .'.');
				return false;
			}
		}
		return true;
	...
	public function render( $wp_customize, $config ) {
		...
		$field_name =  \CustomizerFramework\error_field_name( 'email', $config['id'] );
		...
		if ( is_array( $args ) && parent::sanitize_argument( $config, $field_name ) != false && $is_valid_default == true ) {
			$new_config = \CustomizerFramework\push_default_validation( $config, [ 'valid_email' ] );
			$this->init_settings( $wp_customize, $new_config, $field_name );
			$wp_customize->add_control( new Email_Control( $wp_customize, $args['id'] . '_field', array(
