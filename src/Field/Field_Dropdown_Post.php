 ...
 * @version 1.0.0
 */
final class Field_Dropdown_Post extends Settings
{
	/**
	...
	public function render( $wp_customize, $config ) {
		...
		$field_name =  \CustomizerFramework\error_field_name( 'dropdown-post', $config['id'] );
		...
		if ( is_array( $args ) && parent::sanitize_argument( $config, $field_name ) != false && $is_valid_order == true ) {
			...
			$wp_customize->add_control( new Dropdown_Post_Control( $wp_customize, $args['id'] . '_field', array(
